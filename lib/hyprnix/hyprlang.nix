...
let
  ...
  attrsToNodeList =
    ...
    let
          ...
          mkRepeatNode path name (
            map (
              ...
              if lib.isAttrs value then
                ...
                mkVariableNode path name value
            ) values
          )
        ))
      ];
      sections = lib.pipe attrs [
        ...
        (lib.mapAttrsToList (
          name: value: mkSectionNode path name (attrsToNodeList (path ++ [ name ]) value)
        ))
      ];
    in
    lib.concatLists [
  ...
  pruneEmptyNodesRecursive = lib.foldl' (
    nodes: next:
    let
      next' =
        if isRepeatNode next || isSectionNode next then mapValue pruneEmptyNodesRecursive next else next;
    in
    if next'.value == [ ] then nodes else nodes ++ [ next' ]
  ) [ ];

  sortNodeListRecursive =
    sortPred:
    let
      recurse =
        l:
        lib.pipe l [
          (map (node: if isRepeatNode node || isSectionNode node then mapValue recurse node else node))
          (lib.sort (a: b: sortPred a.path b.path))
        ];
    in
    recurse;

  insertLineBreakNodesRecursive =
    breakPred:
    let
      recurse = lib.foldl' (
        nodes: next:
        let
          prev = if nodes == [ ] then null else builtins.elemAt nodes (builtins.length nodes - 1);
          next' = if isRepeatNode next || isSectionNode next then mapValue recurse next else next;
          newline = mkStringNode next.path "newline" "\n";
        in
        if breakPred prev next' then nodes ++ [ newline ] ++ [ next' ] else nodes ++ [ next' ]
      ) [ ];
    in
    recurse;
    ...
    let
      recurse = lib.foldl' (
        ...
        if isVariableNode next then
          ...
          ++ [
            indent
            (mapValue (v: (recurse v) ++ [ indent ]) next)
          ]
        else
          nodes ++ [ next ]
      ) [ ];
  ...
  renderNode =
    ...
    if isStringNode node then
      ...
      let
        equals = if opts.spaceAroundEquals then " = " else "=";
      in
      ''
