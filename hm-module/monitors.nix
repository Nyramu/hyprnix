...
let
  ...
  mkIntEnum = mapping: {
    ...
    variantName =
      needle:
      (lib.findFirst ({ name, value }: needle == name || needle == value) null (lib.attrsToList mapping))
      .name;
    # Given `needle`, which is either the variant name or an integer value,
    # return the corresponding integer value.
    variantValue =
      needle:
      (lib.find ({ name, value }: needle == name || needle == value) null (lib.attrsToList mapping))
      .value;
    type = types.enum (builtins.attrValues mapping ++ builtins.attrNames mapping);
  ...
  point2DType =
    ...
    types.submodule {
      options = {
        x = lib.mkOption {
          type = numType;
          description = "The X-coordinate of a point, or the width of a rectangle.";
        };
        y = lib.mkOption {
          type = numType;
          description = "The Y-coordinate of a point, or the height of a rectangle.";
        };
      };
    };

  monitorDefType = types.submodule (
    { config, ... }:
    {
      ...
      config =
        let
          ...
          resolutionIsPoint = (point2DType types.ints.positive).check config.resolution;
        in
        {
          output = lib.mkIf (config.description != null) "desc:${config.description}";

          size =
            if resolutionIsPoint then
              let
          ...
          modeString =
            if resolutionIsPoint then
              ...
              "${toString config.resolution.x}x${toString config.resolution.y}${
                lib.optionalString (config.refreshRate != null) "@${toString config.refreshRate}"
              }"
            else
...
in
{
  ...
  config = {
    wayland.windowManager.hyprland.config.monitorv2 = lib.mapAttrsToList (_: def: {
      inherit (def) output;
      ...
      transform =
        let
          default = transformEnum.mapping."Normal";
        in
        lib.mkIf (def.transform != default) def.transform;
