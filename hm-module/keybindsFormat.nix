...
let
  ...
  keyBindsToNodeList =
    ...
    let
      default = lib.pipe attrs [
        (attrs: if attrs ? submap then removeAttrs attrs [ "submap" ] else attrs)
        (bindAttrsToNodeList [ ])
      ];
      submaps = lib.pipe attrs [
        ...
        (lib.mapAttrsToList (
          ...
          in
          mkRepeatNode [ "submap" ] "submap" nodes'
        ))
      ];
    in
    lib.concatLists [
  ...
  bindAttrsToNodeList =
    path:
    (lib.mapAttrsToList (
      bindKw: chordAttrs: mkRepeatNode path bindKw (chordAttrsToNodeList path bindKw chordAttrs)
    ));

  chordAttrsToNodeList =
    path: bindKw: attrs:
    lib.concatLists (
      lib.mapAttrsToList (
        chord: value:
        if lib.isList value then
          (map (dispatcher: mkVariableNode path bindKw "${chord}, ${dispatcher}") value)
        else
          [ (mkVariableNode path bindKw "${chord}, ${value}") ]
      ) attrs
    );
in
{
