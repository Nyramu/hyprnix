          ...
          types.submodule (
            { name, ... }:
            {
              options = {
                ...
                curve = lib.mkOption {
                  type = types.enum ((builtins.attrNames cfg.bezierCurve) ++ [ "default" ]);
                  default = "default";
                };
                style = lib.mkOption {
  ...
  config =
    let
      ...
      stringifyAnimation =
        ...
        "${event}, ${if enable then "1" else "0"}, ${toString (millisToDecis duration)}, ${curve}${
          lib.optionalString (style != null) ", ${style}"
        }";

      stringifyBezier = name: points: "${name}, ${lib.concatStringsSep ", " (map toString points)}";
    in
    {
