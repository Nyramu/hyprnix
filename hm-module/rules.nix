...
in
{
    ...
    wayland.windowManager.hyprland = {
      ...
      workspaceRules = lib.mkOption {
        type =
          ...
          in
          attrsOf (attrsOf (either unitType (listOf unitType)));
        default = { };
        description = lib.mdDoc ''
  ...
  config =
    let
      ...
      compileWindowRulePatterns =
        rule:
        rule
        // {
          class = lib.mapNullable (x: "class:^(${lib.concatStringsSep "|" x})$") rule.class;
          title = lib.mapNullable (x: "title:^(${lib.concatStringsSep "|" x})$") rule.title;
        };
      compileLayerRulePattern =
        rule:
        rule
        // {
      ...
      compileWorkspaceRules =
        rules:
        lib.concatStringsSep ", " (
          lib.mapAttrsToList (
            rule: value:
            if lib.isList value then
              (lib.concatStringsSep ", " (map (it: "${rule}:${valueToString it}") value))
            else
              "${rule}:${valueToString value}"
          ) rules
        );
    in
    {
      ...
      wayland.windowManager.hyprland.config.workspace = lib.pipe cfg.workspaceRules [
