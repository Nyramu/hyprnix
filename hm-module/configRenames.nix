...
let
    ...
    let
      recurse =
        ...
        lib.flatten (
          lib.mapAttrsToList (
            name: value:
            if lib.isAttrs value then (recurse (path ++ [ name ]) value) else mkPathValue path name value
          ) attrs
        );
    in
    recurse [ ];
  ...
  pathValueListToAttrs = lib.foldl' (
    acc: attr: lib.recursiveUpdate acc (lib.setAttrByPath attr.path attr.value)
  ) { };
      ...
      (
        lib.pipe attrs [
          ...
          (map (
            attr:
            let
              idx = lib.indexOf attr.path from;
            in
            if idx == null then
              attr
            else
