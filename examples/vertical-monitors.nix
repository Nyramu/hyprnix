...
{ lib, config, ... }:
{
  wayland.windowManager.hyprland.monitors = with config.wayland.windowManager.hyprland.monitors; {
    ...
    laptop-internal = {
      ...
      position = lib.mapAttrs (_: builtins.floor) {
        ...
        x = desktop-ultrawide.position.x + (desktop-ultrawide.size.x - laptop-internal.size.x) / 2;
